        ...
        <div class="col-xs-12">
            <div class="box">
                <div class="box-header">
                    <h1 class="box-title pullLeft"><b><?php echo $this->lang->line('PRODUCTS_PURCHASE_LABEL'); ?></b></h1>
                    <div class="box-title pullRight">
                        <div class="search-group" id="addButtonDiv">
                            <a href="<?= base_url('admin/manufacturer-products/add') ?>" id="addbutton">
                            <button type="button" class="btn btn-success"><?= $this->lang->line('NEW_PURCHASE_TITLE'); ?></button></a>
                        </div>
                    </div>
                </div>
                    ...
                    <table id="productTable" class="table table-bordered table-hover">
                        <thead>
                            <tr>
                                ...
                                <th><?php echo $this->lang->line('ORDER_TOTAL_DOLLARS_LABEL'); ?></th>
                                <th><?php echo $this->lang->line('ORDER_TOTAL_ISL_LABEL'); ?></th>
                                <th><?php echo $this->lang->line('PAID_PRICE_DOLLARS_LABEL'); ?></th>
                                <th><?php echo $this->lang->line('PAID_PRICE_ISL_LABEL'); ?></th>
                        ...
                        <tbody>
                            ...
                            foreach($productlist as $sl) {  
                                $costpriceInIsl = ($currencyRate['rate']) * $sl->order_total;
                                $paidInIsl = ($currencyRate['rate']) * $sl->paid;
                                $ownInIsl = ($currencyRate['rate']) * $sl->own;
                                ?>
                                <tr>
                                    <!-- <td><?= $sl->name; ?></td> -->
                                    <td><?= $sl->fullname ?></td>
                                    <td><?= ($sl->order_total)? number_format($sl->order_total,2): '' ?></td>
                                    <td><?= number_format($costpriceInIsl)?></td>
                                    <td><?= ($sl->order_total)? number_format($sl->paid,2) : '' ?></td>
                                    <td><?= number_format($paidInIsl,2)?></td>
                                    <td><?= ($sl->own)? number_format($sl->own,2): '' ?></td>
